...
function [Aout,Bout,wout,ucprob,vv,HMC,HRC,CMA,Ephi,HH,ABconverge,mAgap,mBgap] = cmodexog(obsdata,distvar,noj,A,B)
    % This program uses the following inputs
        % obsdata refers to an n by 5 object containing five variables,
            % commercial rent, residential rent, workplace employment, residents, and area        
...
% Extracting variables from the five observed input variables
    qT=obsdata(:,1);    % common observed floor-space rent used in the commercial equation
    QT=obsdata(:,2);    % same common observed floor-space rent used in the residential equation
    HMT=obsdata(:,3);   % workplace employment
    HRT=obsdata(:,4);   % residence employment
    K=obsdata(:,5);     % geographic land area
...
mAgap=max(abs(Agap_r));                                                     % Compute absolute workplace gap
mBgap=max(abs(Bgap_r));                                                     % Compute absolute residence gap
qt_progress_update('Inversion',x,199,max(mAgap,mBgap)./10000,false);
% [x mAgap mBgap];

%%% We stop the iterative procedure if our convergence criteria are met
if (mAgap<=gaptol) && (mBgap<=gaptol);
    qt_progress_update('Inversion',x,199,max(mAgap,mBgap)./10000,true);
...
if ABconverge==0
    qt_progress_update('Inversion',199,199,max(mAgap,mBgap)./10000,false,true);
end
