...
global alpha beta kappa epsilon lambda delta eta rho constructionLandShare;
...
QT=fund(:,5); qT=fund(:,6); HMT=fund(:,7); HRT=fund(:,8);
wT=fund(:,9); vvT=fund(:,10);
thetaT=fund(:,11);

% Use density of development and land area to compute total floor space
L=V.*(K.^constructionLandShare);                                                             % This uses Eq. (15)
...
wage_i=wT;                                                                  % Placeholder for guessed wages, initialized at inverted baseline wages    
...
if (wage_e_r==wage_i_r) & (q_e_r==q_i_r) & (Q_e_r==Q_i_r) & (theta_e_r==theta_i_r);
    qt_progress_update('Closed-city equilibrium',x,999,0,true);
    display('>>>> Convergence Achieved <<<<');
    elapsedMinutes=toc(xtic)./60;
    fprintf('Elapsed minutes: %.2f\\n',elapsedMinutes);
...
qt_progress_update('Closed-city equilibrium',x,999,maxLD,false);     % Compact, Spyder-safe convergence progress
...
if converge==0
    qt_progress_update('Closed-city equilibrium',999,999,maxLD,false,true);
end
