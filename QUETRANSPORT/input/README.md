...
| Network construction | A line layer and a station point layer for each supplied network. Each path pair must be supplied together or both left `null`. A missing baseline network means direct off-network baseline travel; a missing counterfactual network reuses the complete baseline matrix for a fundamentals-only experiment. Speeds are configured in km/h. |
