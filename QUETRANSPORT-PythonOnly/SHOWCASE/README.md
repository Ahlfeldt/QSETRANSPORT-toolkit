...
Add a transport scenario, optionally draw a place-based policy area, edit the
documented configuration file, and press **Run** to produce the maps, tables,
and diagnostics illustrated below.
...
## From inputs to appraisal in one run

### 1. Choose and download a ready-made city grid

![Prime Locations city selector and grid download](input_1_download_grid.png)

Select a city from the map-illustrated dropdown menu and click **Download Grid
Data Shapefile**. The downloadable grids are compatible with QUETRANSPORT, so
users do not need to construct a model geography from scratch. The example
above selects London from the portal for 125 global cities; a parallel portal
covers 381 US metropolitan areas.

### 2. Supply the transport network to evaluate

![Proposed transport lines and stations](input_2_transport_network.png)

Provide separate line and station layers for the **baseline network** and the
**counterfactual network**. The counterfactual therefore need not mean adding
one line to a city with no existing network: it can represent an extension,
replacement, upgrade, rerouting, or removal relative to any supplied baseline.
The toolkit combines each network with the configured speeds and interchange
rules, constructs a complete origin-to-destination travel-time matrix for each
scenario, and evaluates the change between them. A fundamentals-only
application may omit the counterfactual network and retain the complete
baseline travel-time matrix unchanged.

### 3. Optionally draw a place-based change in primitives

![Transport network and a place-based primitive-change polygon](input_3_combined_policy.png)

As a bonus policy component, draw one or more polygons where productivity,
amenities, or structural density (housing supply) should change. This permits,
for example, a joint appraisal of a new transit line and transit-oriented
development around a station. QUETRANSPORT intersects the polygon with the
model grid and applies the configured multiplicative changes to the affected
locations. The primitive-change layer is optional, just like the network
change.

### 4. Configure once and press Run

```text
Downloaded city grid + proposed network + optional policy polygon
                              |
                              v
                    RUN_QUETRANSPORT.py
                              |
                              v
  GRID preparation -> TTMATRIX travel times -> baseline inversion
                              |
                              v
          equilibrium simulations -> maps, tables and diagnostics
```

One user-facing run prepares and validates the spatial inputs, intersects the
grid with the policy polygon, computes travel times, inverts the baseline
economy, simulates the configured counterfactuals, and produces the outputs
below. Intermediate files remain available for validation and reproducibility.
