...
Do not edit generated `input/standardized/runtime_config.json`; it is the
run-specific configuration snapshot consumed by the Python model.

## 3. Run

### Spyder

Open the root `RUN_QUETRANSPORT.py` and run it. The script resolves paths from
its own location and displays preparation, inversion, equilibrium and reporting
progress in Spyder.
...
This is appropriate when preparation and economic simulation should run on
different machines. Copy the complete project with standardized inputs to the
second machine and run `python .\src\python\run_pipeline.py --model-only`.
...
If inversion or equilibrium fails to converge within its configured iteration
limit, the Python model stops rather than saving a successful result.
...
- **Slow network drive:** run from a local or server copy, but preserve the
  project structure and configuration snapshot.
